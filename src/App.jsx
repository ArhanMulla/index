...
import PracticeProjectDetail                 from './pages/PracticeProjectDetail';
...
export default function App() {
  ...
  const loadIdeas = useCallback(async () => {
    ...
    setRealIdeas((rows || []).map(row => {
      ...
      return {
        ...
        author: author?.name || 'INDEX Member', role: author?.role || '', org: author?.org || '',
        author_verified: author?.is_verified || false, author_avatar_url: author?.avatar_url || '',
  ...
  const openProject = project => { setSelProject(project); setView('practiceDetail'); window.scrollTo(0, 0); };
  const openProfileView = profileId => { setPrevView(view); setViewingProfileId(profileId); setView('viewProfile'); window.scrollTo(0, 0); };
  ...
  return (
    <>
      ...
      {view === 'detail'         && <Detail                {...shared} idea={selIdea} onSubmit={() => navTo('submit')} onIdeasChanged={loadIdeas} onViewProfile={openProfileView} />}
      ...
      {view === 'viewProfile'    && <ViewProfile            {...shared} profileId={viewingProfileId} onBack={() => navTo(prevView)} />}
      {view === 'practice'       && <PracticeLibrary        {...shared} onOpenProject={openProject} />}
      {view === 'practiceDetail' && <PracticeProjectDetail  {...shared} project={selProject} onBack={() => navTo('practice')} />}
    </>
  );
}
