...
export default function Submit({ user, view, onNav, onSignOut, onSubmitted }) {
  ...
  const DRAFT_KEY = `index_draft_idea_${user.id}`;

  // Restore an in-progress draft, if one was left behind (e.g. accidental navigation).
  useEffect(() => {
    try {
      const saved = localStorage.getItem(DRAFT_KEY);
      if (saved) {
        const d = JSON.parse(saved);
        setType(d.type ?? 'Industry'); setTitle(d.title ?? ''); setDesc(d.desc ?? '');
        setProblem(d.problem ?? ''); setLimits(d.limits ?? ''); setSkills(d.skills ?? []);
        setTeam(d.team ?? 4); setTl(d.tl ?? '3 months');
        setResearchLink(d.researchLink ?? ''); setWorkLink(d.workLink ?? '');
        if (d.title || d.desc || d.problem) setDraftRestored(true);
      }
  ...
  useEffect(() => {
    const draft = { type, title, desc, problem, limits, skills, team, tl, researchLink, workLink };
    try { localStorage.setItem(DRAFT_KEY, JSON.stringify(draft)); } catch { /* storage full or unavailable — not critical */ }
  }, [type, title, desc, problem, limits, skills, team, tl, researchLink, workLink, DRAFT_KEY]);
      ...
      <main style={{ flex: 1, padding: '40px', background: C.canvas, overflowY: 'auto', fontFamily: F.body }}>
        <div style={{ maxWidth: 660, margin: '0 auto' }}>
          ...
          <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 20, padding: '32px 36px' }}>
            {draftRestored && <div style={{ background: 'rgba(0,191,166,.1)', border: `1.5px solid ${C.signal}`, borderRadius: 10, padding: '10px 14px', color: C.text, fontSize: 12.5, marginBottom: 16 }}>↺ Restored your unfinished draft from last time.</div>}
