  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh' }}>
      ...
      <main style={{ flex: 1, padding: '40px 44px', background: C.canvas, overflowY: 'auto', fontFamily: F.body }}>
        ...
        {ranked.length === 0 && (
          <div style={{ textAlign: 'center', padding: '50px 20px', background: C.surface, border: `1.5px dashed ${C.border}`, borderRadius: 16 }}>
            <div style={{ fontSize: 32, marginBottom: 12 }}>🔍</div>
            <div style={{ fontFamily: F.display, fontSize: 16, fontWeight: 600, color: C.text, marginBottom: 6 }}>No matches yet</div>
            <div style={{ fontSize: 13.5, color: C.muted, marginBottom: 18 }}>Add a few skills to your profile, or check back once more challenges are posted.</div>
            <button onClick={() => onNav('profile')} style={{ padding: '9px 20px', background: C.ink, border: 'none', borderRadius: 999, color: C.onDark, fontSize: 12.5, fontWeight: 600, cursor: 'pointer', fontFamily: F.body }}>Add Skills</button>
          </div>
        )}
      </main>
    </div>
