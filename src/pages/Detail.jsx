...
export default function Detail({ idea, user, view, onNav, onSubmit, onSignOut, onIdeasChanged, onViewProfile }) {
  ...
  const iAmInTeamChat = !isSeed && (isAuthor || myApplication?.status === 'accepted' || applications.some(a => a.applicant_id === user?.id && a.status === 'accepted'));
      ...
      <main style={{ flex: 1, padding: '40px 44px', background: C.canvas, overflowY: 'auto', fontFamily: F.body }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 320px', gap: 16 }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                ...
                {applications.map(app => (
                  <div key={app.id} style={{ border: `1px solid ${C.border}`, borderRadius: 12, padding: '16px 18px', marginBottom: 10 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 10 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 9, cursor: 'pointer' }} onClick={() => onViewProfile(app.applicant_id)}>
            ...
            {iAmInTeamChat && (
              <Chat ideaId={idea.id} myId={user.id} onViewProfile={onViewProfile} />
            )}
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 16, padding: '18px 20px' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: idea.author_id ? 'pointer' : 'default' }} onClick={() => idea.author_id && onViewProfile(idea.author_id)}>
